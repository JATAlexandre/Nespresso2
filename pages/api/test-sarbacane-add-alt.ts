...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    console.log('=== TEST AJOUT DIRECT CONTACT SARBACANE (FORMAT ALTERNATIF) ===');
    ...
    // Format alternatif 1: avec attributes au lieu de fields
    const contactData1 = {
      email: email,
      attributes: {
        phone: phone || ""
    ...
    // Format alternatif 2: avec properties
    const contactData2 = {
      email: email,
      properties: {
        phone: phone || ""
    ...
    // Format alternatif 3: avec custom_fields
    const contactData3 = {
      email: email,
      custom_fields: {
        phone: phone || ""
    ...
    // Format alternatif 4: avec phoneNumber (nom spécifique)
    const contactData4 = {
      email: email,
      phoneNumber: phone || ""
    };
    
    // Format alternatif 5: avec metadata
    const contactData5 = {
      email: email,
      metadata: {
        phone: phone || ""
    ...
    // Utiliser le format 1 pour ce test
    const contactData = contactData1;
    console.log('Données de contact pour l\'API (format alternatif):', JSON.stringify(contactData));
